...
// If query is empty, redirect to blog page
if (empty($query)) {
...
$stmt = $pdo->prepare("SELECT COUNT(*) as total FROM posts p 
                       LEFT JOIN users u ON p.author_id = u.id 
                       LEFT JOIN categories c ON p.category_id = c.id 
                       WHERE p.status = 'published' AND 
                       (p.title LIKE :query OR p.content LIKE :query OR p.excerpt LIKE :query OR 
                        u.first_name LIKE :query OR u.last_name LIKE :query OR c.name LIKE :query)");
...
<div class="container mt-5">
    ...
    <p class="text-muted mb-4"><?php echo $total; ?> result(s) found</p>
